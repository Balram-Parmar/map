...
def record_service():
    if request.method == "GET":
        ...
        records = [{"sepal_length": rec.sepal_length, "sepal_width": rec.sepal_width,
        ...
        db.session.add(pred)
        db.session.commit()
        logger.info(f"Saved prediction: {predicted_class}")
...
@app.route('/health')
def health():
    return jsonify({"status": "up", "service": "db_service"})
